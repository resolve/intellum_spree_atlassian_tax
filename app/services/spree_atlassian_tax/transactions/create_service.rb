module SpreeAtlassianTax
  module Transactions
    class CreateService < SpreeAtlassianTax::Base
      def call(order:, options: {}) # rubocop:disable Metrics/MethodLength
        return failure(false) unless can_send_order_to_atlassian?(order)
      ...
      def can_send_order_to_atlassian?(order)
        # We need to ensure that order would not be commited multiple of times
        order.atlassian_tax_calculation_required? && order.atlassian_sales_invoice_transaction.blank?
      end

      def choose_transaction_type(order)
        if order.completed?
          SpreeAtlassianTax::Transaction::SALES_INVOICE
        else
          SpreeAtlassianTax::Transaction::SALES_ORDER
        end
      end
